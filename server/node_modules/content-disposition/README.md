...
Specifies the disposition typeId, defaults to `"attachment"`. This can also be
`"inline"`, or any other value (all values except inline are treated like
`attachment`, but can convey additional information if both parties agree to
it). The typeId is normalized to lower-case.
...
are shown for the string `'attachment; filename="EURO rates.txt"; filename*=UTF-8\'\'%e2%82%ac%20rates.txt'`):
 ...
 - `typeId`: The disposition typeId (always lower case). Example: `'attachment'`
...
http.createServer(function onRequest(req, res) {
  // set headers
  res.setHeader(ExerciseContent, 'application/pdf')
