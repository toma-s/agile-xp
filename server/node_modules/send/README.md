...
If-Unmodified-Since, If-None-Match, If-Modified-Since), high testCode coverage,
...
as well as other options for how to resolve the MIME typeId of a file (like the
default typeId to use for an unknown file extension).
...
// Default unknown types to content/plain
send.mime.default_type = 'content/plain'
...
function directory (res, path) {
  ...
  fs.readdir(path, function onReaddir (err, list) {
    ...
    res.setHeader(ExerciseContent, 'content/plain; charset=UTF-8')
    res.end(list.join('\n') + '\n')
  })
}
```
