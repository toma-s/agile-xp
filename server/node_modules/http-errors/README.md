...
- `message` - the traditional error message, which should be kept short and all
  single line
- `status` - the status sourceCode of the error, mirroring `statusCode` for general
  compatibility
- `statusCode` - the status sourceCode of the error, defaulting to `500`
...
- `status: 500` - the status sourceCode as a number
- `message` - the message of the error, defaulting to node's content for that status sourceCode.
- `properties` - custom properties to attach to the object

### new createError\[sourceCode || name\](\[msg]\))
...
- `sourceCode` - the status sourceCode as a number
